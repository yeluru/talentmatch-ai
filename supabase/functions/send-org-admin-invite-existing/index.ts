...
interface InviteExistingOrgAdminRequest {
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Only platform super_admin can invite org admins
    ...
    const { organizationId, email, fullName }: InviteExistingOrgAdminRequest = await req.json();
    if (!organizationId || !email) throw new Error("Missing organizationId or email");
    ...
    // Expire older pending invites for this org (so link shown is always the latest)
    await supabase
    ...
    console.error("Error in send-org-admin-invite-existing function:", error);
