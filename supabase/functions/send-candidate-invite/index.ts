...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Only org admins can invite candidates
    ...
    if (!inviterRoles || inviterRoles.length === 0) {
      throw new Error("Only org admins can invite candidates");
    }
    ...
    try {
      ...
      console.error("[audit_logs] failed to write candidate invite log:", e);
    }
    ...
    const emailHtml = `
      ...
      <head>
        <style>
          ...
          .button { display: inline-block; background: linear-gradient(135deg, #10b981, #14b8a6); color: white !important; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; }
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <h2>You're invited to join our talent pool</h2>
            <p>${fullName ? `Hi ${fullName},` : "Hi,"}</p>
            <p><strong>${organizationName}</strong> has invited you to create a candidate profile on UltraHire. You can apply to jobs, manage your resume, and get matched with opportunities.</p>
            <p>Click below to accept the invitation and set up your account:</p>
            <p style="text-align: center; margin: 32px 0;">
              <a href="${inviteUrl}" class="button" style="display: inline-block; background-color: #059669; color: #ffffff !important; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 16px;">Accept Invitation</a>
            </p>
    ...
    const emailResult = await sendEmail(
      email,
      `You're invited to join ${organizationName} on UltraHire`,
      emailHtml,
    );
