...
serve(async (req) => {
  ...
  try {
    ...
    // Uma's info
    const umaUserId = "513ce4f9-2fab-4c33-b5d8-67d891164d54";
    const orgId = "508d1a4e-a02a-4218-a72e-b64520556309";

    // Check Uma's current roles
    const { data: umaRoles } = await supabaseAdmin
      .from('user_roles')
      .select('*')
    ...
    const queries = await Promise.all([
      ...
      // Get candidate_org_links to see who uploaded
      supabaseAdmin.from('candidate_org_links')
        .select('candidate_id, organization_id, created_by, link_type, status, created_at')
        .gte('created_at', new Date(now.getTime() - 14 * 24 * 60 * 60 * 1000).toISOString())
