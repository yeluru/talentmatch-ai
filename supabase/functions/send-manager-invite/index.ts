...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    try {
      ...
      console.error("[audit_logs] failed to write manager invite log:", e);
    }
    ...
    const emailHtml = `
      ...
      <head>
        <style>
          ...
          .button { display: inline-block; background: linear-gradient(135deg, #f59e0b, #fb923c); color: white !important; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; }
    ...
    const emailResult = await sendEmail(
      email,
      `You're invited to join ${organizationName} as a Manager`,
      emailHtml,
    );
    ...
    console.error("Error in send-manager-invite function:", error);
