-- Fix get_talent_pool_candidate_ids to exclude soft-deleted candidates
-- This ensures deleted candidates don't appear in Talent Pool even if they have active org links
...
AS $$
  ...
  sourced AS (
    ...
    WHERE col.status = 'active'
      ...
      AND cp.deleted_at IS NULL  -- Exclude soft-deleted candidates
  ),
  ...
  applicants AS (
    ...
    INNER JOIN public.candidate_profiles cp ON cp.id = a.candidate_id
    WHERE cp.deleted_at IS NULL  -- Exclude soft-deleted candidates
  )
...
COMMENT ON FUNCTION public.get_talent_pool_candidate_ids IS 'Returns candidate IDs for talent pool, excluding soft-deleted candidates';
