...
const GUIDE_SECTIONS: GuideSection[] = [
  {
    ...
    steps: [
      ...
      "Look at the top row of metric cards showing your organization key numbers: total open jobs, total applications received, active candidates in the pipeline, and team size.",
      "Scroll down to view the Pipeline Funnel visualization, which shows how many candidates are at each stage (Engaged to Applied to RTR to Doc Check to Screening to Submission to Outcome).",
  ...
  },
  {
    ...
    steps: [
      ...
      "In the invite dialog, enter the person email address and select their role (Recruiter or Account Manager) from the dropdown.",
  ...
  },
  {
    ...
    description: "See a specific recruiter pipeline and activity (AM oversight).",
    route: "/manager/team",
    overview: "When you are an Account Manager (or manager with oversight), you can view as a specific recruiter to see their pipeline, jobs, and applicants as they see it. This is called Recruiter Progress or view as recruiter. You select a recruiter from the team; the system then shows that recruiter Pipeline or related views so you can coach or audit without taking over their account. Data is read-only in this mode; you are observing, not acting as that user.",
    steps: [
      ...
      "Click on the recruiter name or row to open their team member detail page.",
      "Look for a button labeled View Progress, View as Recruiter, or View Pipeline (usually near the top of their profile).",
      "Click this button to enter the view as mode, where you will see the system from their perspective.",
      "You will now see their specific jobs, their candidate pipeline, their engagements, and their outreach activities, exactly as they see it in Recruiter role.",
      "Navigate through their Pipelines page to see which candidates they are working on for each job, what stages those candidates are in, and when they last took action.",
      ...
      "Use this view to coach the recruiter (e.g., I noticed you have 5 candidates stuck in Screening stage for Job X, let us follow up), audit compliance, or understand workload distribution.",
  ...
  },
  {
    ...
    steps: [
      ...
      "Apply filters to narrow the list: filter by recruiter (to see one person jobs), by status (only open jobs or only closed jobs), or by date range.",
  ...
  },
  {
    ...
    overview: "The Manager Candidates (or Candidates) page shows candidates who are linked to your organization, e.g. they applied to a job, were added to the talent pool, or were engaged by a recruiter. You may see applications across all jobs and recruiters, with filters by job, recruiter, or status. This is useful for ensuring no candidate falls through the cracks or for cross-recruiter reporting. Account Managers may have the same view to see candidates in the org.",
    steps: [
      ...
      "Apply filters to narrow the list: filter by job (to see all candidates for a specific role), by recruiter (to see one person candidates), by status (e.g., only Screening or only Outcome), or by date range.",
      "Click on a candidate name to open their full profile, where you can see their resume, work history, applications across multiple jobs, timeline of interactions, and notes from recruiters.",
      "Review the candidate application timeline to see what stage they are in for each job, when they last moved stages, and what actions recruiters have taken (e.g., sent RTR, scheduled screening).",
  ...
  },
  {
    ...
    steps: [
      ...
      "In the client creation form, enter the client company name, contact person name, email address, phone number, and any notes or additional details.",
      "Click Save or Create Client to add the client to your organization client list.",
      ...
      "View all jobs associated with a client by opening the client detail page and scrolling to the Jobs section, where you will see all open and closed positions for that client.",
  ...
  },
  {
    ...
    overview: "Audit Logs record who did what and when: job created, application status changed, candidate added to pool, invite sent, etc. You can filter by date range, user, action type, or entity (e.g. a specific job or candidate). Use this for compliance, troubleshooting, or understanding how the org uses the platform. Logs are typically read-only; only certain roles (e.g. manager, org admin) can access them.",
    steps: [
      ...
      "Regularly review audit logs to spot unusual patterns, ensure compliance with recruiting processes, or troubleshoot issues (e.g., Who changed this candidate status? or When was this job created?).",
    ],
  },
  {
    id: "am-vs-recruiter",
    ...
    route: "/manager",
    overview: "Account Managers (AMs) have an oversight role: they see org-wide data (team, jobs overview, analytics, audit logs, candidates) and can view as a specific recruiter to see that recruiter pipeline and progress. They do not, by default, see only their own jobs or pipelines, they see the whole org. When an AM switches to Recruiter role (same account, role switch in the UI), they then see only their own recruiter data: their jobs, their applicants, their pipelines, their outreach. The talent pool stays shared. So: use Manager/AM role for oversight and reporting; use Recruiter role for hands-on recruiting.",
    steps: [
      "When signed in as Account Manager, look at the left sidebar menu; you will see Manager pages: Dashboard, Team, Jobs Overview, Candidates, Clients, Organization, and Audit Logs.",
      ...
      "Switch back to Manager/AM role when you need org-wide visibility: reviewing team performance, checking audit logs, managing clients, or viewing a specific recruiter progress for coaching.",
      ...
      "If you are ever unsure which role you are in, check the left sidebar menu (Manager pages equal Manager role; Recruiter pages equal Recruiter role) or the role indicator in the top navigation bar.",
    ],
  },
  {
    id: "team-activity",
    ...
    overview: "The Team Activity Feed shows a live stream of actions taken by recruiters and account managers across your organization. You can see who engaged a candidate, who moved an applicant to a new stage, who created a job, and more, all in real-time. Use this feed for daily oversight, to spot trends, or to understand what your team is working on right now. It is like a social media feed for your recruiting operations.",
    steps: [
  ...
  },
  {
    ...
    description: "Deep-dive into a specific job metrics, applicants, and pipeline.",
    route: "/manager/jobs/:jobId",
    overview: "The Job Detail page gives you a complete view of a single job performance and activity. You can see the full job description, which recruiter owns it, which client it is linked to, how many applications it has received, and the distribution of candidates across pipeline stages. Use this page to understand if a job is performing well, needs more sourcing, or has candidates stuck in a particular stage.",
    steps: [
      ...
      "Use the Edit Job button to update the job description, change the recruiter owner, link a different client, or close the job if it has been filled or canceled.",
  ...
  },
  {
    ...
    overview: "The Recruiter Progress Detail page shows comprehensive performance data for a single recruiter. You can see how many jobs they own, how many candidates they have added to the pool, how many applications they have received, and the distribution of their candidates across pipeline stages. Use this page for performance reviews, coaching sessions, workload balancing, or identifying top performers.",
    steps: [
      ...
      "Click on the recruiter name to open their team member profile.",
      "Click the View Progress or View Detailed Metrics button to open the Recruiter Progress Detail page.",
      "At the top, view the recruiter summary metrics: total jobs owned, total candidates added to the talent pool, total applications (across all their jobs), and current pipeline distribution.",
      ...
      "Review the Pipeline Distribution chart showing how many of this recruiter candidates are at each stage (Engaged, RTR, Screening, etc.), which helps you understand their workload and bottlenecks.",
...
export default function ManagerHowToGuide() {
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col h-[calc(100vh-7rem)] min-h-0 overflow-hidden max-w-[1600px] mx-auto px-2 sm:px-4 lg:px-6 w-full">
        <header className="shrink-0 flex flex-col gap-3">
          ...
          <div className="relative">
            ...
            <Input
              ...
              className="pl-9 h-8 rounded-lg border-border bg-background focus:ring-2 focus:ring-manager/20 font-sans"
            />
          </div>
        </header>
            ...
            <div className="shrink-0 p-4 border-t border-border bg-card">
              <div className="flex gap-2">
                <Input
                  ...
                  disabled={chatLoading}
                  className="flex-1 h-8 rounded-lg border-border focus:ring-2 focus:ring-manager/20 font-sans"
                />
                <Button
                  onClick={sendChat}
                  disabled={chatLoading || !chatInput.trim()}
                  className="shrink-0 h-8 rounded-lg border border-manager/20 bg-manager/10 hover:bg-manager/20 text-manager"
                >
                  {chatLoading ? (
